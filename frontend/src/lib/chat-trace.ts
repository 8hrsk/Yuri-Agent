...
export function reduceRunTrace(trace: RunTrace, event: ChatEvent, at = eventTime(event, nowIso())): RunTrace {
  let next = withUpdatedTrace(trace, at, {
    ...
    maxSteps: event.maxSteps ?? trace.maxSteps,
    maxTokens: event.maxTokens ?? trace.maxTokens,
    maxToolCalls: event.maxToolCalls ?? trace.maxToolCalls,
    maxDurationSeconds: event.maxDurationSeconds ?? trace.maxDurationSeconds,
...
export function normalizeRunTrace(value: unknown, fallbackIndex = 0): RunTrace | undefined {
  ...
  return {
    ...
    maxSteps: nonNegativeInteger(value, 'maxSteps', 'max_steps'),
    maxTokens: nonNegativeInteger(value, 'maxTokens', 'max_tokens'),
    maxToolCalls: nonNegativeInteger(value, 'maxToolCalls', 'max_tool_calls'),
    maxDurationSeconds: nonNegativeInteger(value, 'maxDurationSeconds', 'max_duration_seconds'),
