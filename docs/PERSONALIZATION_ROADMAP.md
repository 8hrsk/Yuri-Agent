...
Реализовано и уточнено после поведенческого тестирования: owner-authored self-description и явно заданные речевые привычки располагаются в начале стилевого контракта и не могут молча исчезнуть из-за character budget. Все 25 стандартных temperament traits, 11 настроек стиля, 7 emotional dynamics, 9 измерений отношения и 13 affects имеют отдельные манифестации на каждом из пяти уровней (very low / low / moderate / high / very high; affect — четыре ступени интенсивности); compiler детерминированно выводит каждую характеристику, а не только наиболее выраженные, располагая выраженные строки первыми. Model-facing текст английский, а речевые маркеры рендерятся на языке ответа; для очень высокой стеснительности заданы двуязычные roleplay-маркеры — заминки, лёгкое заикание, многоточия, самоисправления и смущённые ремарки — с запретом портить грамматику, код и точные данные. Секции affect/relationship добавляются атомарно, чтобы bounded writer не оставлял заголовки с чужим содержимым.
...
- динамические лимиты внутренних диалогов — реализованы четыре среза: per-agent `efficient|balanced|extended` управляет foreground, background, peer и subagent limits; Collaboration позволяет владельцу вручную открыть peer exchange с сужающим override; read-only recommender предлагает лимит по структуре цели и наблюдаемым aggregate-метрикам выбранной пары без model call или скрытой мутации draft; применённая рекомендация получает durable origin и immutable snapshot, а stale/edited preview отклоняется до запуска. Известное context window модели всегда только сужает budget. UI показывает hard ceiling, сравнивает рекомендацию с фактическими turns/tokens/duration/terminal reason и агрегирует завершённые рекомендованные запуски по исторической паре provider/model. До пяти samples одного маршрута калибровка остаётся в состоянии `collecting`; после порога она выдаёт только owner-visible сигнал `tight|balanced|roomy`. Автономного изменения эвристики или hard limits нет. В backlog остаётся проверка этих порогов на накопленной dogfood-истории;
