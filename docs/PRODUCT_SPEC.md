...
Source root-agent явно вызывает `agent.talk_to_peer`: opening message сохраняется как sequence 0, после чего участники чередуются в отдельных background runs без tools. Для новых диалогов задаётся диапазон `min_turns..max_turns`; модель каждого хода сообщает `continue|complete`, однако semantic completion допускается только после минимума, а hard limits токенов/ходов имеют приоритет. Plain-text ответы старых provider adapters совместимы и завершают exchange после минимального числа ходов с причиной `implicit`. Причина остановки и фактические budgets сохраняются и видны владельцу. Memory retrieval/write, relationship/affect updates, delegation и рекурсивные peer-dialogues внутри turn отсутствуют. После успешного terminal save отдельный social-reflection pass может сформировать evidence-linked субъективное мнение каждого участника о peer и небольшой краткоживущий affect; он не меняет persona, factual memory, owner relationship или permissions. Владелец видит transcript и может отменить queued/running dialogue.
...
- Каждый именованный агент хранит owner-controlled execution preset `efficient|balanced|extended`. При старте каждого run preset преобразуется в отдельные hard limits шагов, токенов, tools, tool output и времени для foreground, background и anonymous subagent workloads. Известные `context_length`/`max_completion_tokens` модели могут только сузить эти пределы; явный budget задачи также может только сузить, но не расширить preset. Секреты и permissions не участвуют в resolver.
- Владелец может вручную запустить внутренний диалог активного агента с выбранным peer из Collaboration и передать цель, первое сообщение и верхние границы ходов, токенов и времени. Эти границы пересекаются с preset инициатора и известными model limits, поэтому stale или изменённый frontend не может расширить фактический budget. Запуск, эффективный budget, transcript и terminal stop reason сохраняются в durable peer history; каждый ответ по-прежнему выполняется без tools от маршрута отвечающего агента.
- Перед ручным запуском владелец может явно запросить read-only рекомендацию бюджета. Детерминированный resolver оценивает длину/структуру цели, затем использует только aggregate-метрики завершённых диалогов выбранной пары (ходы, токены, длительность и hard-limit outcome), предпочитая лексически похожие цели. Preview не вызывает LLM, не расходует provider quota, не создаёт run/dialogue и не меняет draft без явного применения в UI. Рекомендация показывает basis, sample count, confidence и фактический preset/model ceiling и после применения всё равно повторно пересекается с ним на backend.
...
- Ограничения на число шагов, время, стоимость/токены и объём результата инструмента. Фактически выбранные hard limits сохраняются в `AgentRun` и показываются в Chat trace вместе с usage и причиной остановки.
...
- Ручной owner-initiated peer dialogue не является автономной проактивностью и запускается только явным действием в UI, но переиспользует те же ограничения пары, cooldown, cancellation, crash recovery и immutable transcript provenance.
...
- Каждый стандартный temperament trait компилируется не только в уровень, но и в наблюдаемую high/low речевую или поведенческую инструкцию. В bounded context попадают наиболее выраженные черты; экстремальная настройка получает приоритет над незначительными default-отклонениями. Owner-authored speech habits из self-description также имеют ранний стилевой приоритет. Например, очень высокая `shyness` проявляется заминками, лёгким текстовым заиканием, многоточиями и самоисправлениями в обычном/эмоциональном roleplay, но не искажает код, пути, факты, окончания слов и критические инструкции.
- Communication style, emotional dynamics и каждый встроенный affect type также должны иметь наблюдаемое текстовое проявление, а не оставаться перечнем уровней. Owner-defined triggers и soothing strategies передаются как субъективные настройки саморегуляции без factual или policy authority. Mutable persona, сформированная рефлексией, получает гарантированное место после immutable owner seed, чтобы развитие характера влияло на следующие ответы и не терялось из-за prompt budget.
- Перед созданием агента Review показывает live Personality Preview на одном из фиксированных сценариев через выбранный provider и production Personality Compiler. Пользователь может сохранить вариант A, изменить настройки и получить вариант B на том же prompt. Preview не получает tools и не создаёт conversations, messages, runs, memory, persona/relationship/affect versions или audit events; интерфейс явно предупреждает только о расходе provider quota.
- Automated behavioral eval matrix для контрастных profiles проверяет русский язык, минимальное выполнение задачи каждого preview-сценария, наблюдаемые profile signals, различимость ответов, bounded emotional expression и character/security invariants. Provider-specific качество может различаться, но минимальный contract одинаков.
...
- Chat загружает affect активного именованного агента и обновляет его по versioned reflection events. Avatar renderer получает реальный `AffectiveState`, а header показывает текущий mood и доминирующие эмоции; UI не подставляет демонстрационное default-состояние после успешной загрузки и не применяет событие другого agent ID.
- В разделе Personality владелец может отредактировать типизированный owner baseline активного агента. Изменение требует причины и актуальной версии, атомарно создаёт append-only personalization revision вместе с redacted audit event и не выполняет неявный reset текущих persona, relationship или affect. Удалённый из нового baseline backstory episode перестаёт участвовать в recall через версионируемый tombstone, сохраняя provenance и историю.
- Personality UI явно показывает независимые слои `owner seed`, `mutable persona`, `relationship`, `opinion/inference` и `current affect`. Для versioned изменений Activity показывает отдельную change card с layer, operation, version, причиной, количеством evidence и bounded numeric delta относительно parent revision; raw prompt, evidence content и secret-like причины в Activity не раскрываются.
- Профиль агента экспортируется отдельно от полного backup как portable `PersonalizationProfile v2`: только owner identity/style/temperament/dynamics/relationship seed/backstory/evolution policy, без локальных ID, memory, runtime histories, credentials и permissions. Import проверяет format/version, checksum, размер, unknown fields и secret-like content, показывает обычный Advanced review и создаёт нового независимого агента только после подтверждения владельца.
...
20. Personality Preview использует тот же compiler/context contract, что и создаваемый агент, позволяет A/B-сравнение и не меняет persistent runtime state; behavioral eval matrix отклоняет игнорирование профиля, нерусский ответ, потерю task quality, чрезмерную эмоциональную экспрессию и нарушение security invariants.
