...
Yuri может читать, искать и изменять файлы только внутри директорий, разрешённых пользователем. Если model tool call указывает на путь вне текущих корней, приложение показывает канонический абсолютный путь и запрашивает решение: разрешить один раз, сохранить доступ к этой директории или запретить. После разрешения продолжается тот же tool call без повторного запроса к модели. Перед перезаписью или удалением дополнительно показываются точный путь, характер изменения и возможность отмены, если она технически доступна.
...
- Ответы агента отображаются как безопасный GitHub Flavored Markdown: заголовки, списки, цитаты, inline/fenced code и таблицы. Raw HTML не исполняется, небезопасные URL-схемы отклоняются, а удалённые изображения из текста модели не загружаются автоматически. HTTP(S)-ссылки и Markdown-ссылки на существующие абсолютные локальные пути открываются только после явного клика пользователя через отдельные desktop bridge-команды, без навигации WebView.
- Вложения: произвольные UTF-8 текстовые/кодовые файлы (включая Markdown, JSON и исходный код) и PNG/JPEG/GIF/WebP. Бинарные executable/archive/object formats отклоняются. Один turn принимает до 6 файлов: до 1 MiB на текстовый файл, до 4 MiB на изображение и до 5 MiB суммарно. Метаданные и hash сохраняются с сообщением, payload — в content-addressed blob storage; изображения передаются provider adapter как native multimodal parts.
...
- Просмотр, пауза, ручной запуск, остановка активного run, редактирование и удаление.
- До введения обязательного durable idempotency contract для каждого изменяющего tool автоматические retry/lease recovery выполняют только low-risk read tools; side effects запускаются интерактивно с подтверждением.
...
- Счётчики дневного лимита, cooldown и идентификаторы доставленных уведомлений восстанавливаются из append-only audit после restart; deferred delivery имеет детерминированный idempotency key.
...
- Начальный owner `RelationshipState` создаётся из выбранного relationship seed конкретного агента. Custom scenario остаётся субъективной вымышленной предысторией с provenance на owner seed, а не фактом о пользователе. Trust/attachment из temperament являются только predisposition; накопленные trust/attachment принадлежат отдельной связи с конкретным subject. Owner может видеть причины и дельты версий, выполнить append-only rollback или reset к текущему seed без изменения persona, affect, memory и других relationships. Peer relationships направлены и получают отдельный нейтрально-субъективный seed из temperament observer-а, не наследуя owner-specific romance/backstory.
