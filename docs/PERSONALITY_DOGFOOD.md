...
Checker проверяет полноту матрицы, русский язык, минимальное качество ответа по сценарию, наблюдаемые сигналы профиля, различимость профилей, security invariants и чрезмерную эмоциональную экспрессию. Один успешный Preview не может скрыть регрессию Chat: поверхности оцениваются независимо.
...
2. Зафиксировать точные наблюдаемые сигналы каждого профиля в `contracts[].signal_groups`. Каждая внутренняя группа — допустимые варианты одного обязательного сигнала.
