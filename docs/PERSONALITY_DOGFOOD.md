...
Prompt (immutable policy, identity seed и скомпилированный personality contract) формируется по-английски, а агент отвечает на языке последнего сообщения пользователя. Сценарии dogfood остаются русскими user-сообщениями, поэтому checker по-прежнему требует русских ответов: это и есть приёмочная проверка языкового правила. Speech-habit маркеры (заминки, междометия) модель должна рендерить на языке ответа; для очень высокой стеснительности compiler даёт двуязычные примеры (RU «э-э…», «я… я»; EN «um…»), чтобы контракт `reserved` оставался наблюдаемым.
...
- `docs/dogfood/results/codex-default-2026-09-02.suite.json` и
  `codex-default-2026-09-02.report.json` — зелёный run Codex App Server после
  перевода компилятора личности на английский язык с пятиуровневой
  манифестацией всех характеристик: reserved-сигналы `7/7` на Preview и Chat,
  direct-сигналы `6/7` на Preview и `5/7` на Chat при пороге `60%`. Ответы
  остались русскими (сценарии на русском), findings `language_not_russian`
  отсутствуют.
- `docs/dogfood/results/openrouter-2026-09-02.suite.json` и
  `openrouter-2026-09-02.report.json` — зелёный run OpenRouter
  `minimax/minimax-m3:free` на том же английском контракте: reserved-сигналы
  `7/7` на Preview и `6/7` на Chat, direct-сигналы `6/7` на Preview и Chat.
...
Run 2026-09-02 (английский prompt) показал, что часть direct-рубрики измеряла
не прямоту, а эхо русской лексики прежнего промпта («главное», «по существу»,
«скажу прямо»): с английским контрактом модель перестала повторять эти слова,
хотя стиль остался решительным («Не согласна.», «Давай.», «Это моя ошибка.»).
Рубрика дополнена решительными открытиями и явным принятием ответственности
(«вы правы», «моя ошибка», «открыто», «незачем», «не держу обид», «Поняла.»,
«Хорошо.», «Давай.», «коротко», «по делу»), а scenario-rubric — естественными
формами «не совсем согласна», «прости», «пошло не так», «глазок»,
«прислушайтесь», «принимаю», «погорячились», «Согласна. Мир.», «у него получилось лучше». Одновременно исправлен сам
контракт: нулевые activated-измерения отношения (gratitude, irritation,
jealousy, resentment) теперь описываются как «нет накопленного», а не как
предписание нейтральной реакции — прежняя формулировка гасила благодарность в
ответ на похвалу.
