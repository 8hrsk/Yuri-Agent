...
Provider-independent `Personality Compiler` объединяет owner seed с текущими `MutablePersona`, `RelationshipState` и `AffectiveState`, применяет owner bounds и преобразует числа в воспроизводимые качественные уровни и конкретные правила речи. Весь model-facing текст компилятора, immutable policy и identity seed написан по-английски; язык ответа агента определяется языком последнего сообщения пользователя, а `PreferredLanguage` owner seed используется только как fallback для неоднозначных сообщений. Каждая характеристика всех слоёв (25 temperament traits, 11 настроек стиля, 7 emotional dynamics, 9 измерений отношения, 13 affects) попадает в контракт с пятиуровневым шагом (very low / low / moderate / high / very high; для affect — четыре ступени интенсивности), где выраженные уровни получают детальную манифестацию (речевые маркеры, ситуации проявления, границу), а moderate — короткую строку. Он явно различает predisposition, отношение к текущему subject и transient affect, разрешает известные конфликты traits и всегда включает invariants качества/безопасности. Dialogue runtime получает только bounded compiled data envelope до 20 000 символов (типичный профиль ≈ 6–7 тысяч, полный all-extreme ≈ 15 тысяч); точные raw values остаются в diagnostic snapshot и typed storage. Compiler не импортирует provider, tool registry или policy engine и по конструкции не может расширить permissions.
