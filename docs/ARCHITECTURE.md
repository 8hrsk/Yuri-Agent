...
Personality dogfood не имеет доступа к runtime storage или provider credentials. Versioned suite хранит только публичные provider/model labels, фиксированные scenario IDs, profile contracts и финальные тексты ответов. `cmd/yuri-personality-eval` строгим decoder-ом проверяет одинаковую матрицу для isolated Preview и production Chat, затем применяет общий provider-independent behavioral checker отдельно к каждой поверхности. Offline fixture является canary формата, а не заявленным результатом внешней модели; authenticated runs запускаются владельцем явно и не должны содержать приватные transcripts.
