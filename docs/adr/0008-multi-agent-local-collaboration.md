...
15. Калибровка не меняет hard limits автоматически. UI сравнивает immutable рекомендацию с фактическими ходами, токенами, duration и terminal reason, а сводку строит только по завершённым `owner_recommendation` exchanges и их историческим provider/model routes.
