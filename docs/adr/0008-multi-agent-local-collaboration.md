...
13. Владелец может явно открыть тот же peer-dialogue aggregate из Collaboration. Запрос содержит bounded purpose/opening message и необязательные верхние границы turns/tokens/duration; resolver пересекает их с preset инициатора и model metadata. Owner override никогда не расширяет hard limits и не выдаёт tools, permissions или доступ к чужой private memory.
14. Budget recommendation является отдельным read-only preview: deterministic heuristic сначала оценивает bounded purpose, затем может использовать только aggregate-метрики завершённых exchanges той же пары. Похожесть определяется локально без embeddings/model call; transcript content, private memory и credentials в recommender не передаются. Результат содержит provenance/confidence и остаётся неприменённым до явного действия владельца. Если владелец применяет preview, запуск атомарно сохраняет `owner_recommendation` и точный снимок budget/basis/sample count; перед созданием aggregate backend пересчитывает рекомендацию и отклоняет stale/edited значения. Для `owner_custom` и `agent_default` снимок отсутствует.
15. Калибровка не меняет hard limits автоматически. UI сравнивает immutable рекомендацию с фактическими ходами, токенами, duration и terminal reason, а сводку строит только по завершённым `owner_recommendation` exchanges и их историческим provider/model routes. До пяти samples маршрут имеет только состояние `collecting`; `tight|balanced|roomy` — read-only dogfood-сигнал после минимальной выборки, а не runtime policy.
...
- Ручной запуск владельца переиспользует этот aggregate/runtime и отличается объяснимой trigger reason. UI показывает эффективные лимиты до/после запуска; сервер повторно валидирует payload и выполняет чистое `min(base, override)` для каждого заданного upper bound.
- Recommender не является policy boundary: renderer дополнительно зажимает preview для корректного отображения, но только повторный server-side `min(resolved ceiling, owner override)` перед persistence является обязательной защитой от расширения бюджета.
