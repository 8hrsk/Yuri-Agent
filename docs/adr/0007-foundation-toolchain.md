...
5. Обычная проверка выполняется через `make check`. Полная universal macOS OSS-сборка запускается через `make macos-smoke`; target сам использует закреплённый Wails CLI из `.tools/`, валидирует app metadata/архитектуры и создаёт SHA-256 manifest. `make macos-launch-smoke` отдельно проверяет `OnDomReady` и graceful shutdown собранного `.app`, а `make macos-ui-smoke` и `make macos-voice-smoke` выполняют детерминированные onboarding/voice interactions внутри native Wails WebKit. Все targets используют `open(1)` и изолированный временный profile root без внешнего WebDriver.
