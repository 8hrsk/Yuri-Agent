...
| T-13 | Reflection/runaway persona drift меняет Yuri слишком быстро или через вредный контент | High | Immutable seed/policy, bounded trait delta, cooldown, evidence threshold, one reflection at a time, version log, rollback/reset, no external tools | Медленные неверные изменения возможны при систематически ошибочном feedback |
...
| T-28 | Peer message меняет policy/state другого агента, запускает рекурсивный dialogue или создаёт runaway background loop | High | Named participants only; root tool trigger; one reply; pair concurrency + 5-minute cooldown; TTL/token/time budget; empty tool registry; no context assembler/memory/persona writes; peer transcript marked untrusted; captured participants/provider; cancel and no-retry crash recovery; redacted audit | Инициатор может передать peer чувствительный текст, который увидит выбранный cloud provider и владелец в локальном Collaboration transcript |
