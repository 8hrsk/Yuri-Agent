...
| T-05 | Path traversal, symlink, alias или junction обходят разрешённый корень | High | Normalize + resolve real path, проверка root containment до и во время операции, запрет неожиданных links, path audit | TOCTOU и OS-specific edge cases требуют hardening на macOS |
...
| T-10 | Cross-session retrieval раскрывает эпизод не в том контексте | High | Single-owner context still scope by task; retrieval budget, relevance threshold, sensitivity filters, provenance, explicit session search | При локальном профиле любой foreground run потенциально видит разрешённую общую память |
