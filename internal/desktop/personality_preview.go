...
func generatePersonalityPreview(ctx context.Context, backend agent.ModelBackend, model string, input PersonalityPreviewInput) (PersonalityPreviewView, error) {
	...
	snapshot, err := assembler.Assemble(ctx, contextbuilder.Input{
		...
		IdentitySeed:      agentIdentitySeed(state.Profile, []domain.AgentProfile{state.Profile}, state.Personalization.Identity.PreferredLanguage),
		ProjectContext:    "PERSONALITY PREVIEW: answer one test scenario as this character. Do not mention the preview, settings or internal parameters. Tools are unavailable; never claim an external action was performed.",
